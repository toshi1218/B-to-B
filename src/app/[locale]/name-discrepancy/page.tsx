...
import { AlertCircle, Check, FileText, UserX, Search } from "lucide-react";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "metadata.name_discrepancy" });
  return { title: t("title"), description: t("description") };
}

const painIcons = [UserX, AlertCircle, FileText, Search];
const painColors = [
...
const painIconColors = ["text-red-600", "text-orange-600", "text-yellow-600", "text-purple-600"];
...
export default async function NameDiscrepancyPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: "name_discrepancy_page" });
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-[#1a2846] to-[#273c69] py-14 text-white">
        <div className="container-site">
          <div className="mb-4 flex items-center gap-3">
            <UserX className="text-blue-300" size={36} />
          </div>
      ...
      {/* Common name discrepancy examples */}
      <div className="bg-gray-50 py-16">
        <div className="container-site">
          <h2 className="mb-4 text-2xl font-bold text-gray-900">{t("examples_heading")}</h2>
          <p className="mb-8 text-gray-600 leading-relaxed max-w-2xl">{t("examples_intro")}</p>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {exampleItems.map((key) => (
              <div
                key={key}
