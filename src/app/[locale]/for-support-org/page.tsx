...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "metadata.for_support_org" });
...
export default async function ForSupportOrgPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: "for_support_org_page" });
      ...
      <div className="bg-[#f0f3f9] py-16">
        <div className="container-site">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              <Building2 className="mb-4 text-[#1a2846]" size={28} />
              <h3 className="mb-2 font-bold text-gray-900">{t("service4_title")}</h3>
              <p className="mb-4 text-sm text-gray-600 leading-relaxed">{t("service4_desc")}</p>
              <Link
                href={`/${locale}/contact`}
                className="inline-flex items-center gap-1 text-sm font-semibold text-[#1a2846] hover:underline"
              >
