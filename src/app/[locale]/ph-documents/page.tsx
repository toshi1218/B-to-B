...
import { FileText, AlertCircle, Check, Globe, Package } from "lucide-react";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "metadata.ph_documents" });
  return { title: t("title"), description: t("description") };
}

const painIcons = [AlertCircle, Globe, FileText, Package];
const painColors = [
...
export default async function PhDocumentsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: "ph_documents_page" });
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-[#1a2846] to-[#273c69] py-14 text-white">
        <div className="container-site">
          <div className="mb-4 flex items-center gap-3">
            <FileText className="text-blue-300" size={36} />
          </div>
      ...
      <div className="bg-white py-16">
        <div className="container-site">
          <h2 className="mb-8 text-2xl font-bold text-gray-900">{t("documents_heading")}</h2>
          <p className="mb-8 text-gray-600 leading-relaxed max-w-2xl">{t("documents_intro")}</p>
      ...
      <div className="bg-gray-50 py-16">
        <div className="container-site">
          ...
          <p className="text-gray-700 leading-relaxed max-w-3xl">{t("services_desc")}</p>
        </div>
      </div>
